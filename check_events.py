...
def send_via_webhook(webhook_url: str, content: str) -> None:
...
def send_via_bot(token: str, channel_id: str, content: str) -> None:
...
def main() -> None:
    ...
    if webhook_url:
        send = lambda content: send_via_webhook(webhook_url, content)
    elif token and channel_id:
        send = lambda content: send_via_bot(token, channel_id, content)
    else:
        print(
